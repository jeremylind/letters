...
var wordSegmentLength = rollerWordsCount - 1;
var scrollerOffset = 0.55;

export default function scrollInit() {
	var position = setScale(percentInView(), 1 / (1 - scrollerOffset), scrollerOffset);
	position = trim(position, 1, -99999);
	for (var i = rollerWords.length - 1; i >= 0; i--) {
		// var wordPosition = setScale2(position, 5, i * 9 / 10);
		var wordPosition = setScale(position, wordSegmentLength, ((i / wordSegmentLength) - (1 / (wordSegmentLength * 2))));
...
function getBlur(x) {
	var position = -3 * Math.pow((x - 0.5), 2) + 1;
	position = setScale(position, 0.5, -1);
	return trim((1 - position), 1, 0) * 10;
}
